-- 098 — Owner-editable booking request aside (platform presets + custom items)

alter table public.public_site_settings
  add column if not exists booking_notice jsonb not null default '{}'::jsonb;

comment on column public.public_site_settings.booking_notice is
  'Modular "good to know" panel on the public booking page: presets and custom items.';

drop function if exists public.upsert_public_site_settings_atomic(
  uuid, text, text, boolean, boolean, text, boolean, jsonb, jsonb, jsonb, jsonb
);

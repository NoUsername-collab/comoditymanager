...
import { DEV_LEVEL_COLOR } from "@/lib/hospira-admin/log-styles";
...
function DevLogDetails({ log }: { log: PlatformDevLogEntry }) {
  ...
  return (
    <div className="hospira-devlog-entry__details mt-2 space-y-2 border-t border-neutral-700/80 pt-2 text-xs text-neutral-400">
      {log.request_path && (
        <p>
...
export function PlatformDevLogTable({ logs }: { logs: PlatformDevLogEntry[] }) {
  ...
  return (
    <div
      id="dev-logs"
      className="hospira-log-section scroll-mt-6 rounded-lg border border-neutral-800 bg-neutral-900 p-3.5"
    >
      ...
      <ul className="hospira-log-cards max-h-[min(70dvh,36rem)] space-y-2 overflow-y-auto">
        {filteredLogs.map((log) => (
          <li key={log.id}>
            <details
              className={`hospira-log-card group rounded-md border px-3 py-2.5 ${rowHighlightClass(log.level)} ${
      ...
      <div className="hospira-log-table-desktop hospira-logs-table-wrap max-h-[500px] overflow-y-auto">

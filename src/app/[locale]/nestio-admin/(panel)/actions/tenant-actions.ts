...
import { runPlatformAdminAction } from "@/lib/nestio-admin/platform-action";
import {
  updateTenantPlan,
  updateTenantStatus,
  updateTenantModules,
...
export async function changeTenantPlanAction(
  ...
  const run = await runPlatformAdminAction(async () => {
    ...
    revalidatePath("/nestio-admin");
    revalidatePath("/nestio-admin/tenants");
    revalidatePath(`/nestio-admin/tenants/${tenantId}`);
    return result;
  });

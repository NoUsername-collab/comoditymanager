...
import { isNestioAdminDbError } from "@/lib/nestio-admin/format-db-error";
...
export default function NestioLogsError({
  ...
  const t = useTranslations("nestioAdmin.error");
  const logsT = useTranslations("nestioAdmin.logs.error");

  useEffect(() => {
    console.error("[nestio-admin/logs]", {
      name: error.name,
      message: error.message,
      digest: error.digest,
      stack: error.stack,
      ...(isNestioAdminDbError(error)
        ? {
  ...
  return (
    <div className="nestio-admin-error flex min-h-[min(50dvh,28rem)] items-center justify-center px-4 py-5">
      <div className="nestio-admin-error__card max-w-2xl rounded-lg border border-red-900 bg-red-950/30 p-4 text-center">
        ...
        <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-center">
          ...
          <Link
            href="/nestio-admin/logs#dev-logs"

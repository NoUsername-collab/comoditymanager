...
export default async function BuildingsPage({
  ...
  return (
    <AdminRetroPageFrame
      title="Clădiri — Casa Emil"
      description="Alege o dată — vezi camere libere / ocupate pe clădire și în grid."
      action={{ href: "/admin/buildings/new", label: "+ Adaugă clădire" }}
    >
      ...
      <div className="mt-6 space-y-4">
        {dashboards.map((d) => (
          <RetroXpWindow
            key={d.building.id}
            title={`${d.building.name} — etaje & camere`}
          >
      ...
      {dashboards.length === 0 && !error && (
        <p className="mt-8 text-center text-zinc-500">
          Nicio clădire — adaugă prima din butonul de sus.
        </p>
      )}
    </AdminRetroPageFrame>
  );

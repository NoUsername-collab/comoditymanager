...
export class HospiraAdminDbError extends Error {
...
export function throwIfDbError(
  ...
  if (error) {
    throw new HospiraAdminDbError(context, error);
  }
}

/** Works across RSC → client error boundaries where instanceof breaks. */
export function isHospiraAdminDbError(
  error: unknown
): error is HospiraAdminDbError {
  if (error instanceof HospiraAdminDbError) return true;
  return error instanceof Error && error.name === "HospiraAdminDbError";
}

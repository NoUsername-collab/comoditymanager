...
export async function updateCheckin(
  ...
  if (identityResult.blockers.length > 0) {
    throw new Error(`checkin.blocked: ${identityResult.blockers.join("; ")}`);
  }
  ...
  if (deleteGuestsErr) throw new Error(deleteGuestsErr.message);
  ...
  await logAdminActivityFromSession({
    ...
    entityType: "checkin",
    entityId: checkinId,
    summary: `Check-in actualizat: ${booking.guest_name}${flagSummary}`,
    metadata: {
